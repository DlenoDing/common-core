...
- HTTP Controller / WS Controller 示例**刻意不直接声明** `#[AutoController]` / `#[WsController]`,避免误扫后注册真实路由;复制到业务项目后再按注释添加。
...
- `Middleware/`:HTTP 模块前置中间件示例,用于接入登录校验、防重放和自定义签名钩子;复制后需在业务 `dependencies.php` 绑定 `AbstractModuleBeforeMiddleware`。
- `Model/`:普通模型和分表模型示例。
- `Process/`:自定义常驻进程示例。
