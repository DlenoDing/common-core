...
class Xtea
{
    ...
    public static function encrypt(
        string $data,
        array $keys,
        int $padding_scheme = self::PAD_0x00,
        int $rounds = 32
    ): string {
     ...
     * @return string
     */
    public static function encrypt_unsafe(
        string $data,
        array $keys,
        int $padding_scheme = self::PAD_0x00,
        int $rounds = 32
    ...
    public static function decrypt(string $data, array $keys, int $rounds = 32): string
    {
        ...
        if ($len < 8) {
            throw new \InvalidArgumentException(
                "this cannot be (intact) xtea-encrypted data, it's less than 8 bytes long (the minimum xtea length)"
            );
        }
        if (($len % 8) !== 0) {
            throw new \InvalidArgumentException(
...
$data = "233____aabbcc"; // MDAwMDAxNzkAcUBfX18EYSxIY3NzN2Y0NjBkZDdlYmQ2ZWJkMThhNTEwOGUyZTQxYjE2ZmE=
//print_r($data);
$encrypted = XTEA::encrypt($data, $keys_array, XTEA::PAD_0x00);
hex_dump($encrypted);
$decrypted = XTEA::decrypt($encrypted, $keys_array);
//var_dump($data, $encrypted, $decrypted);

print_r("over the xtea \n\n\n\n\n");*/
