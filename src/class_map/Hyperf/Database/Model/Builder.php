...
class Builder
{
    ...
    public function __call($method, $parameters)
    {
        ...
        $object = $this->__execCall($method, $parameters);
        //执行完成后恢复别名
        if (in_array($method, $insertMethods)) {
            $model          = $this->getModel();
            $model->isAlias = true;
     ...
     * @param array $parameters
     */
    private function __execCall($method, $parameters)
    {
        if ($method === 'macro') {
    ...
    {
        return $this->toBase()
                    ->update($this->addUpdatedAtColumn($values));
    }
